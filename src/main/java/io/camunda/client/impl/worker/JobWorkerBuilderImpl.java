...
public final class JobWorkerBuilderImpl
  ...
  private static final org.slf4j.Logger LOG =
      org.slf4j.LoggerFactory.getLogger(JobWorkerBuilderImpl.class);
  ...
  // Optional Falcon hook: non-null when built via patched CamundaClientImpl.
  ...
  public JobWorkerBuilderImpl(
    ...
    this(configuration, jobClient, scheduledExecutor, jobHandlingExecutor, closeables, null);
  }
  ...
  public JobWorker open() {
    ...
    // Falcon upgrade (Nano push): always prefer Falcon when the gateway
    // advertises it and the WS is live. On subscribe failure fall through to
    // the stock REST/gRPC path — matches JS/Rust SDK semantics.
    if (falconAwareClient != null) {
      final com.nanobpm.camunda.falcon.FalconTransport falcon =
          falconAwareClient.falconTransport();
      if (falcon != null) {
        try {
          final FalconJobWorker worker =
              new FalconJobWorker(
                  falcon,
                  jobType,
                  workerName,
                  ...
                  jobClient,
                  jobHandlingExecutor,
                  falconAwareClient.jsonMapper());
          worker.open();
          closeables.add(worker);
          return worker;
        } catch (final Throwable e) {
          LOG.warn(
              "Falcon subscribe failed ({}); falling back to REST/gRPC job worker",
              e.getMessage());
        }
      }
    }
